...
g5 <- g4 +
  geom_text(aes(x = Country_o, y = Income.inequality + 0.2, 
                label = format(Income.inequality, digits = 3, nsmall = 2))) 
# g5
g6 <- g5 +
  labs(title = "소득 불평등(5분위배율)", x = "나라", y = "소득 5분위배율") +
  theme(plot.title = element_text(hjust = 0.5, size = 15, family = "KoPubWorldDotum Bold"),
...
p4 <- p2 + 
  geom_text(hjust = hjust_text, family = "KoPubWorldDotum Medium")
p4
```
...
p5 <- p2 + 
  geom_text(hjust = hjust_text, vjust = vjust_text, family = "KoPubWorldDotum Medium")
p5
```
...
p6 <- p2 + 
  geom_text(hjust = hjust_text, vjust = vjust_text, 
            nudge_x = nudge_x_text, nudge_y = nudge_y_text, family = "KoPubWorldDotum Medium")
p6
```
