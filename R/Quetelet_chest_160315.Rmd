...
`r knitr::include_graphics("../pics/quetelet_soldiers.png", dpi = 150)`
...
h_chest.2 <- hist(chest_long, 
                  breaks = 32.5:48.5, 
                  plot = FALSE)
list(breaks = h_chest.2$breaks, 
     counts = h_chest.2$counts, 
     density = h_chest.2$density, 
     mids = h_chest.2$mids)
```
...
y.norm <- dnorm(x.chest, mean = mean_chest, sd = sd_chest)
hist(chest_long, breaks = 32.5:48.5, probability = TRUE, main = main_title, sub = sub_title, xlab = x_lab, ylab = y_lab)
...
hist(chest_long, breaks = 32.5:48.5, probability = TRUE, main = main_title, sub = sub_title, xlab = x_lab, ylab = y_lab, axes = FALSE)
abline(v = c(x_lower, x_upper), lty = 2, col = "red")
polygon(poly_df, density = 20)
# polygon(x_coord, y_coord, density = 20)
lines(x.chest, y.norm, col = "red")
...
g0 <- ggplot(data = data.frame(chest_long), mapping = aes(x = chest_long)) 
(g1 <- g0 +
...
(g2 <- g1 + 
   geom_vline(xintercept = c(x_lower, x_upper), linetype = "dotted", colour = "red")) 
```
...
(g4 <- g3 + 
   geom_polygon(data = poly_df, mapping = aes(x = x, y = y), alpha = 0.5, fill = "grey"))
```
...
# y.curve <- dnorm(x.curve, mean = mean_chest, sd = sd_chest)
