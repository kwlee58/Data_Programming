...
plot(Index_inequality_df, pch = 20)
text(Index_inequality_df, labels = Country)
```
...
plot(Index_inequality_df, pch = 20)
text(Index_inequality_df, labels = Country, pos = 1)
```
...
plot(Index_inequality_df, pch = 20)
text(Index_inequality_df, labels = Country, pos = 2)
```
...
plot(Index_inequality_df, pch = 20)
text(Index_inequality_df, labels = Country, pos = 3)
```
...
plot(Index_inequality_df, pch = 20)
text(Index_inequality_df, labels = Country, pos = 4)
```
...
plot(Index_inequality_df, pch = 20, xlim = c(3, 9), ylim = c(-1.5, 2.5))
text(Index_inequality_df, labels = Country, pos = 4, cex = 0.8)
```
...
text_left
pos_text <- ifelse(1:nrow(data_21) %in% text_up, 3, ifelse(1:nrow(data_21) %in% text_left, 2, 4))
pos_text
```
...
title(main = main_title, xlab = x_lab, ylab = y_lab)
mtext(c("Better", "Worse"), side = 2, at = c(-1.8, 2.8), las = 1)
text(x = 5, y = 1.5, labels = paste("r =", round(cor_1, digits = 2)), col = "blue")
