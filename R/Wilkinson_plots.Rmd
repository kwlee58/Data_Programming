...
```{r, base plot}
Index_inequality.df <- data_21[c("Income_inequality", "Index_HS")]
str(Index_inequality.df)
plot(Index_inequality.df)
cor(Index_inequality.df)
cor.1 <- cor(data_21["Income_inequality"], data_21["Index_HS"])
cor.1
```

매우 높은 양의 상관관계(r = `r cor.1`) 가 관찰됨을 알 수 있다. 자주 사용하는 `data_21[c("Income_inequality", "Index_HS")]`를 간단한 R 오브젝트로 assign하여 반복 사용하고 있다. `cor()`에도 data frame을 사용하면 어떻게 되는지 다음 결과와 비교해 보자.
...
lm.ineq <- lm(Index_HS ~ Income_inequality, data = Index_inequality.df)
...
Index_GDP.df <- data_21[c("GDP_WB", "Index_HS")]
...
cor.2 <- cor(Index_GDP.df["GDP_WB"], Index_GDP.df["Index_HS"])
